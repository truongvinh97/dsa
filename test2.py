...
FIRMWARE_FILE = "firmware.hex"  # Path to the firmware file (.hex format)
CHUNK_SIZE = 512                # Number of bytes per OTA chunk
NEW_FW_VERSION = "0x0102"       # New firmware version to be sent

def load_firmware_hex(filename):
    """
    Parses an Intel HEX file and returns its binary content.
    This simple parser handles data records (record type 00) and
    extended linear address records (record type 04). It fills any missing
    bytes with 0xFF.
    """
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if record_type == 0:  # Data record
                ...
                if absolute_address + byte_count > highest_address:
                    highest_address = absolute_address + byte_count
...
def publish_ota_update(client, firmware_data):
    ...
    for i in range(num_chunks):
        ...
        # Convert binary chunk to a hex string (uppercase)
...
def main():
    # Load firmware data (either from a .hex or .bin file)
    try:
    ...
    # Start publishing OTA update messages
